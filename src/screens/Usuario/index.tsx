		...
		<Fragment>
				...
				<View style={tw`flex-1 mx-18 bg-white justify-center`}>
					<View style={tw`flex-row justify-around px-15`}>
						<TouchableOpacity style={tw`h-12 w-12 rounded border items-center justify-center border-slate-400 bg-white`}>
							<Image style={tw`h-7 w-7`} source={require('D:/Projetos/app-bico/src/images/servico.png')} />
						</TouchableOpacity>
						<TouchableOpacity style={tw`h-12 w-12 rounded items-center justify-center border border-slate-400 bg-white`}>
							<Image style={tw`h-7 w-7`} source={require('D:/Projetos/app-bico/src/images/camera.png')} />
						</TouchableOpacity>
					</View>
				</View>
			</View>
			...
			<View style={tw`flex-1 bg-gray-50 pb-40`}>
				...
				<View style={tw`py-2 mt-5 items-center justify-center`}>
					<TextInput style={tw`h-16 px-25 border-solid border-[0.50] border-slate-400 rounded-md`} placeholder='Escolha seu serviço' value={search} onChangeText={setSearch}></TextInput>
				</View>
				<View style={tw`py-10 justify-between`}>
					<ScrollView>
						{list.length === 0 ? (
							<View style={tw`flex-1 justify-center items-center pt-[20%]`}>
								<Text style={tw`text-base text-slate-600`}>Nenhum resultado encontrado</Text>
							</View>
						) : (
							<>
								{list.map(item => (
